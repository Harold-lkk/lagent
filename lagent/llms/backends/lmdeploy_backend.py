...
class LMDeployBackend(LocalBackend):
    ...
    def init_client(self, model: str, backend_config: LMDeployBackendConfig, gen_params) -> Callable:
        from lmdeploy import ChatTemplateConfig, TurbomindEngineConfig, pipeline

        model_name = backend_config.pop('model_name', None)
        chat_template_config = ChatTemplateConfig(model_name=model_name) if model_name else None
    ...
    def _completion(
        ...
        **gen_params,
        ...
        response = self.model.batch_infer(inputs, gen_config=gen_config, do_preprocess=False)
        return response
