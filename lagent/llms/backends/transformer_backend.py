...
class HFTransformer(BaseLLM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(path=path, tokenizer_only=tokenizer_only, meta_template=meta_template, **kwargs)
        if isinstance(stop_words_id, int):
            stop_words_id = [stop_words_id]
        self.gen_params.update(stop_words_id=stop_words_id)
        if self.gen_params['stop_words'] is not None and self.gen_params['stop_words_id'] is not None:
            logger.warning('Both stop_words and stop_words_id are specified,' 'only stop_words_id will be used.')
    ...
    def _load_tokenizer(self, path: str, tokenizer_path: Optional[str], tokenizer_kwargs: dict):
        ...
        if self.tokenizer.pad_token_id is None:
            if self.tokenizer.eos_token is not None:
                logger.warning(f'Using eos_token_id {self.tokenizer.eos_token} ' 'as pad_token_id.')
                ...
                if self.gcfg.pad_token_id is not None:
                    logger.warning(f'Using pad_token_id {self.gcfg.pad_token_id} ' 'as pad_token_id.')
    ...
    def tokenize(self, inputs: str):
        ...
        inputs = self.tokenizer(inputs, return_tensors='pt', return_length=True)
        ...
        with torch.no_grad():
            ...
            while True:
                ...
                for i in range(len(output_token_ids)):
                    ...
                    first_eos_idx = next(
                        (idx for idx, token_id in enumerate(output_token_ids[i]) if token_id in eos_token_id), None
                    )
