...
class LocalBackend:
    ...
    def chat_completion(
        ...
        **gen_params,
    ) -> Union[str, List[str], ChatCompletionResponse,
        ...
        formatted_responses = self.response_to_chat_completion_response(
            responses)
        if is_batched:
            return formatted_responses
        return formatted_responses[0]
    ...
    def apply_chat_template(self, inputs):
        chat_template = self.chat_template or self.gen_params.get(
            'chat_template')
        if chat_template:
